        ...
        if is_speech_result:
            ...
            if self.silence_duration >= 1 and len(self.buffer) * CHUNK_DURATION >= MIN_SPEECH_DURATION:
                ...
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmpfile:
                    ...
                    print(f"💾 寫入暫存 WAV 檔耗時: {write_end - write_start:.2f}s")

                    diarization_start = time.perf_counter()
                    result = self.diarization_pipeline(tmpfile.name)
                    diarization_end = time.perf_counter()
                    print(f"🗣️ Diarization 耗時: {diarization_end - diarization_start:.2f}s")
